...
#ifdef _DEBUG
		WriteConsoleW(consoleOut, fname[0].namebody, fname[0].name_size, &k, NULL); // вывод имени найденного файла
		WriteConsoleW(consoleOut, L"\r\n", 2, &k, NULL);
#endif
...
#ifdef _DEBUG
		WriteConsoleW(consoleOut, fname[0].namebody, fname[0].name_size, &written, NULL); // вывод найденного файлового объекта (всех по порядку, пока не нашли нужный)
		WriteConsoleW(consoleOut, L"\r\n", 2, &written, NULL);
#endif
